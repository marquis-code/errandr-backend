/**
 * Seed Facilitators Script
 * 
 * Usage: npx ts-node scripts/seed_facilitators.ts
 * 
 * This script creates facilitator records and sends welcome emails
 * with branded referral codes to each campus ambassador.
 */
...
const FACILITATORS = [
  {
    name: 'Ogazi Favour',
    email: 'ogazichisom01@gmail.com',
    matricNumber: '210706026',
    skill: 'Graphics Design',
  },
  {
    name: 'Williams Afolabi',
    email: 'afolabioluwatimileyinwilliams@gmail.com',
    matricNumber: '210706052',
    skill: 'UI/UX Designer',
  },
  {
    name: 'Babalola Eniola',
    email: 'eniolababs123@gmail.com',
    matricNumber: '210706017',
    skill: 'Graphics Design',
  },
  {
    name: 'Abdulrahman Al-ameen',
    email: 'ayomideabdulrahman2204@gmail.com',
    matricNumber: '210707038',
    skill: 'Product Manager',
  },
  {
    name: 'Isaac-Taiwo Pearl',
    email: 'pearlisaac32@gmail.com', // Fixed from .coml
    matricNumber: '',
    skill: 'Content Creation',
  },
  {
    name: 'Oduntan Oluwatoni',
    email: 'oluwatoni.oduntan@gmail.com',
    matricNumber: '230707520',
    skill: 'Graphics Design and Video Editing',
  },
  {
    name: 'Emeji Success',
    email: 'princesssuccess391@gmail.com',
    matricNumber: '210706008',
    skill: 'Product Manager',
  },
  {
    name: 'Ajayi Atilola Ruth',
    email: 'ajayiatilola03@gmail.com',
    matricNumber: '210708099',
    skill: 'Product Manager',
  },
  {
    name: 'Oyewole Goodness Boluwaji',
    email: 'oyewolegoodness444@gmail.com',
    matricNumber: '210708084',
    skill: 'Content Creation',
  },
  {
    name: 'Oluwafemi Favour Eniola',
    email: 'favouroluwafemi24@gmail.com',
    matricNumber: '240705510',
    skill: 'Copywriting',
  },
  {
    name: 'Sunmola Ruth Oluwaseun',
    email: 'sunmolaruth@gmail.com',
    matricNumber: '210707050',
    skill: 'Social Media Management and Content Creation',
  },
  {
    name: 'Abah Marquis',
...
// ─── Mongoose Schemas (inline for standalone script) ─────────────
...
// ─── Generate Branded Referral Code ──────────────────────────────
...
async function main() {
  ...
  const emailFrom = process.env.EMAIL_FROM?.replace(/['"]+/g, '') || 'Erranders <notifications@erranders.org>';
  ...
  console.log('\n🚀 Erranders Facilitator Seed Script');
  console.log('━'.repeat(50));
  console.log(`📦 ${FACILITATORS.length} facilitators to seed\n`);
  ...
  if (resendApiKey) {
    ...
    console.log('⚠️  RESEND_API_KEY not found — emails will be skipped\n');
  }

  const results: any[] = [];
  const existingCodes = new Set<string>();

  // Pre-fetch existing facilitators to avoid duplicates
  const existingFacilitators = await FacilitatorModel.find({});
  const existingEmails = new Set(existingFacilitators.map(f => f.email));
  existingFacilitators.forEach(f => existingCodes.add(f.referralCode as string));

  for (const f of FACILITATORS) {
    try {
      // Skip if already seeded
      if (existingEmails.has(f.email)) {
        console.log(`⏭️  ${f.name} (${f.email}) — already exists, skipping`);
        results.push({ success: true, name: f.name, status: 'already_exists' });
        continue;
      }

      // Generate unique branded code
      let code = generateBrandedCode(f.name);
      ...
      console.log(`✅ Created: ${f.name} → Code: ${code}`);

      // Send welcome email
      if (resend) {
        try {
          const html = buildWelcomeEmail(f.name, code, f.skill);
          await resend.emails.send({
            from: emailFrom,
            to: [f.email],
            subject: `Welcome to the Squad, ${f.name.split(' ')[0]}! 💜 Your Code: ${code}`,
            html,
          });

          facilitator.welcomeEmailSent = true;
          await facilitator.save();
          console.log(`   📧 Welcome email sent to ${f.email}`);
        } catch (emailErr: any) {
          console.error(`   ❌ Email failed for ${f.email}: ${emailErr.message}`);
        }
      }

      results.push({ success: true, name: f.name, code, email: f.email });
    } catch (err: any) {
  ...
  for (const r of successful) {
    if (r.code) {
      console.log(`   ${r.name.padEnd(30)} → ${r.code}`);
    } else {
      console.log(`   ${r.name.padEnd(30)} → (already existed)`);
    }
  }
