...
export function isFoodVendor(vendor: any): boolean {
  const type = (vendor?.businessType || vendor?.storeType || vendor?.vendorType || '').toLowerCase();
  const cat = (vendor?.category || '').toLowerCase();
  
  return FOOD_VENDOR_CATEGORIES.includes(cat as any) || type === 'food' || type === 'restaurant';
}
